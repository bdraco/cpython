...
def run_benchmarks():
    ...
    for state_name, setup_func in states.items():
        ...
        speedup = time_with_lock / time_lockless if time_lockless > 0 else float('inf')
        print(f"{state_name:10} {time_with_lock:10.2f} ns {time_lockless:10.2f} ns {speedup:8.2f}x")
